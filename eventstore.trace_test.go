...
//err := s.store.Save("", []IEventMessage{}, nil, nil)
//c.Assert(err, Equals, ErrNoEventsToAppend)
//s.store.StopTracing()
...
//c.Assert(err, IsNil)
//s.store.StartTracing()
//err = s.store.Save("", []IEventMessage{event2}, nil, nil)
//c.Assert(err, IsNil)
//s.store.StopTracing()
//trace := s.store.GetTrace()
//c.Assert(trace, HasLen, 1)
...
//event1 := &TestEvent{yooid(), "event1"}
//store.StartTracing()
//err := store.Save("", []IEventMessage{event1}, nil, nil)
//c.Assert(err, IsNil)
//store.StopTracing()
...
//store := NewTraceEventStore(nil)
//events, err := store.Load(yooid().String())
